...
class Line(object):
    ...
    def line_plus(self, other_line):
        ...
        new_x = self.start.x + other_line.start.x
        new_y = self.start.y + other_line.start.y
        newx = self.end.x + other_line.end.x
        newy = self.end.y + other_line.end.y

        return Line(Point(new_x, new_y), Point(newx, newy))
    ...
    def midpoint(self):
        ...
        avgx = (self.start.x + self.end.x)/2
        avgy = (self.start.y + self.end.y)/2
    ...
    def is_parallel(self, line2):
        ...
        if line2.start.x == line2.end.x:
            ...
            lslope = (line2.start.y - line2.end.y) / (line2.start.x - line2.end.x)

        if round(slope, 12) == round(lslope, 12):
            return True
        else:
            return False
